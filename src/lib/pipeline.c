...
Error execute(State_t *state) {
  ...
  uint32_t nzcv
    = extract_bits((uint32_t) *cpsr, CPSR_BIT_LOWER, CPSR_BIT_UPPER);
...
uint32_t get_ins_type( uint32_t code) {
  uint32_t res = extract_bits(code,TYPE_BIT_LOWER,TYPE_BIT_UPPER);
  if (res == FURTHER_TYPE_CHECKING) {
    uint32_t first_value
      = extract_bits(code, MULTIPLY_FIRST_BIT_LOWER, MULTIPLY_FIRST_BIT_UPPER);
    uint32_t second_value
      = extract_bits(code, MULTIPLY_SECOND_BIT_LOWER, MULTIPLY_SECOND_BIT_UPPER);
